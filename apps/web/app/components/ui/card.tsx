...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn("bg-white text-aic-navy flex flex-col gap-6 rounded-xl border border-gray-200", className)}
      {...props}
    />
  );
}
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return <p data-slot="card-description" className={cn("text-aic-muted", className)} {...props} />;
}
