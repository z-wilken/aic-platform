...
export async function POST(request: Request) {
  try {
    ...
    // 1. Insert or Update leads table (Lead management is unified)
    await db
      .insert(leads)
      .values({
        email,
    ...
    // Note: alpha_applications table is managed via leads status now
    // to maintain a lean institutional schema.
